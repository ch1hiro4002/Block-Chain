...
import (
	...
	"ch1hiro4002/blockchain/network"
...
func main() {
	...
	hasPrivateKey, _ := strconv.ParseBool(envOr("HAS_PRIVATE_KEY", "true"))
	...
	if hasPrivateKey {
		generatedKey := crypto.GeneratePrivateKey()
	...
	srv, err := network.NewServer(opts, addr, pubKey, stake)
	if err != nil {
		logrus.Fatal(err)
	}
